...
<?php $__env->startSection('content'); ?>
    <div class="max-w-4xl mx-auto px-4 py-8 relative min-h-screen">
        ...
        <?php if($cartItems->isEmpty()): ?>
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-12 text-center mt-10">
                ...
                <a href="<?php echo e(route('handphone.index')); ?>" class="inline-block bg-orange-500 hover:bg-orange-600 shadow-lg shadow-orange-200 text-white font-bold py-3 px-8 rounded-xl transition-transform hover:-translate-y-1">
                    Mulai Belanja
                </a>
            </div>
        <?php else: ?>
            ...
            <div class="space-y-4 mb-8">
                <?php $__currentLoopData = $cartItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex items-center gap-4 transition-all hover:border-orange-300 hover:shadow-md cart-item" 
                         ...
                         data-price="<?php echo e($item->product->price ?? 0); ?>">
                        
                        <!-- Checkbox Barang -->
                        <input type="checkbox" class="item-checkbox w-5 h-5 rounded border-gray-300 text-orange-500 focus:ring-orange-500 z-10 cursor-pointer" name="selected_items[]" value="<?php echo e($item->id); ?>">

                        <!-- LINK MENUJU DETAIL PRODUK -->
                        <a href="<?php echo e($item->product ? route('product.show', $item->product->id) : '#'); ?>" class="flex flex-1 items-center gap-4 group">
                            <!-- Gambar Produk -->
                            <div class="w-20 h-20 bg-gray-50 rounded-xl border border-gray-100 flex items-center justify-center overflow-hidden flex-shrink-0 group-hover:opacity-80 transition-opacity">
                                <img src="<?php echo e($item->product->image_url ?? asset('images/products/default.jpg')); ?>" class="w-full h-full object-contain p-1">
                            </div>

                            <!-- Detail Produk -->
                            <div class="flex-1 min-w-0">
                                <div class="text-xs font-bold text-blue-600 mb-0.5"><?php echo e($item->product->brand->name ?? 'Unbranded'); ?></div>
                                ...
                                <p class="text-orange-500 font-black mt-1">
                                    Rp <?php echo e(number_format($item->product->price ?? 0, 0, ',', '.')); ?>
                        ...
                        <div class="flex flex-col items-end gap-3 z-10">
                            <!-- Form Hapus Satuan -->
                            <form action="<?php echo e(route('cart.remove', $item->id)); ?>" method="POST" onsubmit="return confirm('Hapus produk ini?')">
                            ...
                            <div class="flex items-center border border-gray-200 rounded-lg overflow-hidden bg-white shadow-sm">
                                <button onclick="updateQty(<?php echo e($item->id); ?>, -1)" class="w-8 h-8 flex items-center justify-center hover:bg-gray-100 text-gray-600 font-bold transition-colors">-</button>
                                <span id="qty-<?php echo e($item->id); ?>" class="w-8 h-8 flex items-center justify-center bg-gray-50 text-sm font-bold border-x border-gray-200"><?php echo e($item->quantity); ?></span>
                                <button onclick="updateQty(<?php echo e($item->id); ?>, 1)" class="w-8 h-8 flex items-center justify-center hover:bg-gray-100 text-gray-600 font-bold transition-colors">+</button>
                            </div>
                        </div>
                    </div>
    ...
    <form id="bulk-delete-form" action="<?php echo e(route('cart.bulk-delete')); ?>" method="POST" class="hidden">
        <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\projek pak fajar\market-handphone\resources\views/user/cart.blade.php ENDPATH**/ ?>