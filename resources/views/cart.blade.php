            ...
            <div class="space-y-4 mb-8">
                @foreach($cartItems as $item)
                    <div class="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex items-center gap-4 transition-all hover:border-orange-300 hover:shadow-md cart-item" 
                        ...
                        <a href="/product/{{ $item->product->id ?? '#' }}" class="flex flex-1 items-center gap-4 group">
                            <!-- Gambar Produk -->
                        ...
                        <div class="flex flex-col items-end gap-3 z-10">
                            <!-- Form Hapus Satuan -->
                            <form action="/cart/remove/{{ $item->id }}" method="POST" onsubmit="return confirm('Hapus produk ini?')">
                                @csrf @method('DELETE')
