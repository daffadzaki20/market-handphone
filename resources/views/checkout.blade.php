...
@section('content')
            ...
            <div class="p-6">
                ...
                <div class="flex justify-between items-start gap-4 text-gray-800 text-sm">
                    ...
                    @else
                        ...
                        <a href="{{ route('alamat.index') ?? '/profile' }}" class="bg-orange-500 text-white px-4 py-2.5 rounded-lg font-bold uppercase text-xs hover:bg-orange-600 transition-all shadow-sm">
                            + TAMBAH
                        </a>
                    @endif
                </div>
                ...
                @foreach($checkoutItems as $item)
                    <div class="flex items-center gap-4 border-b border-gray-50 pb-4 last:border-0 last:pb-0">
                        @if(isset($item->product->image))
                            <img src="{{ asset('storage/' . $item->product->image) }}" class="w-16 h-16 object-cover border border-gray-200 bg-gray-50 rounded">
                        @else
                            <div class="w-16 h-16 bg-gray-100 border border-gray-200 flex items-center justify-center rounded">
    ...
    <div class="fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 shadow-[0_-5px_15px_rgba(0,0,0,0.05)] z-50">
        <div class="max-w-5xl mx-auto flex justify-end items-stretch">
            ...
            <div class="flex items-stretch justify-end w-full md:w-auto">
                ...
                <form id="form-buat-pesanan" action="/checkout/process" method="POST" class="flex">
                    @csrf
                    <!-- Data Barang -->
                    @foreach($checkoutItems as $item)
    ...
    @if(isset($semuaAlamat) && count($semuaAlamat) > 0)
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div class="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl overflow-hidden">
                ...
                <div class="p-4 max-h-[60vh] overflow-y-auto space-y-3 bg-gray-50/30">
                    @foreach($semuaAlamat as $item)
                    <div class="relative border-2 rounded-xl p-4 cursor-pointer transition-all {{ isset($alamatUtama) && $item->id == $alamatUtama->id ? 'border-orange-500 bg-orange-50/30 shadow-sm' : 'border-transparent bg-white hover:border-gray-200 shadow-sm' }}" 
                         onclick="pilihAlamatIni('{{ $item->id }}')">
                        
                        @if(isset($alamatUtama) && $item->id == $alamatUtama->id)
                            <div class="absolute top-4 right-4 text-orange-500">
                                <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path></svg>
                            </div>
                        @endif

                        <p class="font-bold text-gray-800">{{ $item->user->name ?? 'Pengguna' }} <span class="text-gray-400 font-normal">| {{ $item->user->phone_number ?? '-' }}</span></p>
                        <p class="text-sm text-gray-600 mt-2 leading-relaxed">
                            <span class="font-semibold text-gray-800 bg-gray-100 px-2 py-0.5 rounded text-xs mr-1">{{ $item->label }}</span> 
                            {{ $item->alamat_detail }}, RT.{{ $item->rt }}/RW.{{ $item->rw }}, {{ $item->desa }}, {{ $item->kecamatan }}, {{ $item->kabupaten }}, {{ $item->provinsi }}
                        </p>
                    </div>
                    @endforeach
                </div>
                ...
                <div class="p-4 border-t border-gray-100 bg-white flex justify-center">
                    ...
                    <a href="{{ route('alamat.index') ?? '/profile' }}" class="text-orange-500 font-bold text-sm hover:underline flex items-center gap-1">
