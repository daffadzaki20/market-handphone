...
<div class="space-y-3">
    @foreach($alamats as $alamat)
        {{-- Menambahkan 'relative' agar icon absolute di dalamnya terposisi dengan benar --}}
        <div class="relative border border-orange-200 rounded-lg p-4 hover:shadow-md bg-white transition-all group">
            ...
            <div class="flex justify-between items-start mb-2 pr-10"> {{-- pr-10 agar tidak menabrak icon --}}
                <div class="flex items-center gap-2">
                    <span class="font-bold text-gray-800 text-base">{{ $alamat->user->name }}</span>
                    <span class="text-gray-300">|</span>
                    <span class="text-gray-600 text-sm">{{ $alamat->user->phone_number }}</span>
                </div>
            ...
            <p class="text-sm text-gray-600 leading-relaxed mb-3 pr-10">
                <span class="font-medium text-gray-800">{{ $alamat->label ?? 'Alamat' }}:</span> 
                {{ $alamat->alamat_detail }}, 
                @if($alamat->rt && $alamat->rw) RT.{{ $alamat->rt }}/RW.{{ $alamat->rw }}, @endif
                {{ $alamat->desa }}, {{ $alamat->kecamatan }}, {{ $alamat->kabupaten }}, {{ $alamat->provinsi }}, {{ $alamat->kode_pos }}
            </p>
            
            <!-- 4. AKSI (LIHAT PETA, UBAH, HAPUS) -->
            <div class="flex items-center justify-between mt-3 border-t border-gray-50 pt-3">
                <div class="flex items-center gap-4 text-sm font-medium">
                    <a href="https://www.google.com/maps?q={{ $alamat->latitude }},{{ $alamat->longitude }}" target="_blank" class="text-blue-500 hover:text-blue-600 hover:underline flex items-center gap-1.5 cursor-pointer">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                        Lihat di Peta
                    </a>
                    <span class="text-gray-300">|</span>
                    <button onclick="editAlamat({{ json_encode($alamat) }})" 
                        class="text-orange-500 hover:text-orange-600 cursor-pointer">
                        Ubah
                    </button>
                </div>
                
                {{-- Tombol Hapus diletakkan di kanan bawah untuk kemudahan akses --}}
...
<div id="modal-alamat" class="fixed inset-0 z-[9999] hidden bg-black bg-opacity-60 flex items-start justify-center p-4 pt-16 md:pt-24 overflow-y-auto opacity-0 transition-opacity duration-300">
    ...
    <div class="bg-white rounded-lg shadow-xl w-full max-w-4xl flex flex-col overflow-hidden transform scale-95 transition-transform duration-300 mb-10" id="modal-content">
        ...
        <form id="form-alamat" action="{{ route('alamat.store') }}" method="POST" class="space-y-6">
        @csrf
