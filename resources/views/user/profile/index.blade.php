        ...
        <div class="flex-1 bg-white shadow-sm border border-gray-100 rounded-md p-6 md:p-8">  
            ...
            <div class="flex flex-col-reverse md:flex-row gap-8">
                ...
                <div class="flex-1 md:pr-12 md:border-r border-gray-200">
                    ...
                    <form action="{{ route('profile.update') }}" method="POST">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center mb-6">
                            ...
                            <div class="sm:w-2/3">
                                <input type="text" name="name" value="{{ Auth::user()->name }}" class="w-full border border-gray-300 rounded-sm px-3 py-2 text-sm focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none transition-colors">
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center mb-6">
                            ...
                            <div class="sm:w-2/3 text-sm text-gray-800">
                                <input type="email" name="email" value="{{ Auth::user()->email }}" placeholder="Masukkan email" class="w-full border border-gray-300 rounded-sm px-3 py-2 text-sm focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none transition-colors">
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center mb-6">
                            ...
                            <div class="sm:w-2/3 text-sm">
                                <input type="tel" name="phone_number" value="{{ Auth::user()->phone_number }}" placeholder="Contoh: 081234567890" minlength="10" maxlength="15" oninput="this.value = this.value.replace(/[^0-9]/g, '')" class="w-full border border-gray-300 rounded-sm px-3 py-2 text-sm focus:border-orange-500 outline-none">
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center mb-6">
                            ...
                            <div class="sm:w-2/3 flex gap-4 text-sm text-gray-700">
                                <label class="flex items-center gap-1 cursor-pointer"><input type="radio" name="gender" value="Laki-laki" {{ Auth::user()->gender == 'Laki-laki' ? 'checked' : '' }} class="text-orange-500 focus:ring-orange-500"> Laki-laki</label>
                                <label class="flex items-center gap-1 cursor-pointer"><input type="radio" name="gender" value="Perempuan" {{ Auth::user()->gender == 'Perempuan' ? 'checked' : '' }} class="text-orange-500 focus:ring-orange-500"> Perempuan</label>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center mb-8">
                            ...
                            <div class="sm:w-2/3 flex gap-2">
                                ...
                                <select name="dob_day" class="border border-gray-300 rounded-sm px-2 py-2 text-sm text-gray-700 focus:border-orange-500 outline-none w-20">
                                    <option value="" disabled {{ !$savedDay ? 'selected' : '' }}>Hari</option>
                                    @for ($i = 1; $i <= 31; $i++)
                                        <option value="{{ $i }}" {{ $savedDay == $i ? 'selected' : '' }}>{{ $i }}</option>
                                    @endfor
                                </select>
                                ...
                                <select name="dob_month" class="border border-gray-300 rounded-sm px-2 py-2 text-sm text-gray-700 focus:border-orange-500 outline-none w-32">
                                    <option value="" disabled {{ !$savedMonth ? 'selected' : '' }}>Bulan</option>
                                    @foreach ($bulan as $angka => $nama)
                                        <option value="{{ $angka }}" {{ $savedMonth == $angka ? 'selected' : '' }}>{{ $nama }}</option>
                                    @endforeach
                                </select>
                                ...
                                <select name="dob_year" class="border border-gray-300 rounded-sm px-2 py-2 text-sm text-gray-700 focus:border-orange-500 outline-none w-24">
                                    <option value="" disabled {{ !$savedYear ? 'selected' : '' }}>Tahun</option>
                                    @for ($i = date('Y'); $i >= 1950; $i--)
                                        <option value="{{ $i }}" {{ $savedYear == $i ? 'selected' : '' }}>{{ $i }}</option>
                                    @endfor
                                </select>
                ...
                <div class="w-full md:w-1/3 flex flex-col items-center pt-2">
                    ...
                    <form action="{{ route('profile.photo') }}" method="POST" enctype="multipart/form-data" id="photoForm" class="flex flex-col items-center">
                        @csrf
                        ...
                        <div class="w-28 h-28 mb-5 relative group">
                            @if(Auth::user()->profile_photo)
                                <img src="{{ asset('storage/' . Auth::user()->profile_photo) }}" alt="Foto Profil" class="w-full h-full rounded-full object-cover border border-gray-200 shadow-sm">
                            @else
