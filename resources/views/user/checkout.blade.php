...
@section('content')
            ...
            <div class="p-6">
                ...
                <div class="flex justify-between items-start gap-4 text-gray-800 text-sm">
                        ...
                        <div class="flex-1 relative bg-gray-50 rounded-xl p-4 border border-gray-100">
                            ...
                            <div class="flex flex-col gap-1 pr-10">
                                <p class="font-bold text-base text-gray-800">
                                    {{ $alamatUtama->user->name ?? 'Pengguna' }} 
                                    <span class="font-normal text-gray-600 text-sm ml-1">{{ $alamatUtama->user->phone_number ?? '-' }}</span>
                                </p>
                                <p class="text-gray-600 text-sm leading-relaxed">
                                    <span class="font-semibold text-gray-700">[{{ $alamatUtama->label }}]</span> 
                                    {{ $alamatUtama->alamat_detail }}, 
                                    @if($alamatUtama->rt || $alamatUtama->rw)
                                        RT.{{ $alamatUtama->rt }}/RW.{{ $alamatUtama->rw }}, 
                                    @endif
                                    {{ $alamatUtama->desa }}, {{ $alamatUtama->kecamatan }}, 
                    ...
                    @else
                        ...
                        <a href="{{ route('alamat.index') }}" class="bg-orange-500 text-white px-4 py-2.5 rounded-lg font-bold uppercase text-xs hover:bg-orange-600 transition-all shadow-sm">
                            + TAMBAH
                        </a>
                    @endif
                </div>
            ...
            <div class="space-y-4">
                @foreach($checkoutItems as $item)
                    <div class="flex items-center gap-4 border-b border-gray-50 pb-4 last:border-0 last:pb-0">
                        <img src="{{ $item->product->image_url ?? asset('images/products/default.jpg') }}" class="w-16 h-16 object-cover border border-gray-200 bg-gray-50 rounded">
                        <div class="flex-1 min-w-0">
                            <h3 class="text-gray-800 text-sm truncate font-medium">{{ $item->product->name }}</h3>
                            <p class="text-xs text-blue-500 font-bold uppercase mt-0.5">{{ $item->product->brand->name ?? '' }}</p>
                        </div>
                        ...
                        @if(isset($myVouchers))
                            @foreach($myVouchers as $v)
                                ...
                                <option value="{{ $v->id }}" 
                                        ...
                                        data-type="{{ $v->type }}" 
                                        data-value="{{ $v->value }}" 
                                        data-min="{{ $v->min_spend }}"
                                        {{ !$memenuhiSyarat ? 'disabled' : '' }}>
                                    [{{ $v->code }}] 
                                    @if($v->type == 'percent')
                                        Diskon {{ $v->value }}%
                                    @else
                                    ...
                                    @endif 
                                    (Min. Belanja Rp {{ number_format($v->min_spend, 0, ',', '.') }})
                                    {{ !$memenuhiSyarat ? ' - (Belum Memenuhi Syarat)' : '' }}
                                </option>
                            @endforeach
                        @endif
                    </select>
                    ...
                    <label class="relative cursor-pointer select-none">
                        ...
                        <div class="opsi-box px-5 py-3 border border-gray-200 text-gray-700 rounded-xl transition min-w-[220px]">
                            <div class="flex flex-col">
                                ...
                                <span class="text-xs text-gray-500 mt-1.5 font-medium">Estimasi tiba <span class="text-gray-700 font-bold">{{ now()->addDays(2)->translatedFormat('d') }} - {{ now()->addDays(4)->translatedFormat('d M') }}</span></span>
                    ...
                    <label class="relative cursor-pointer select-none">
                        ...
                        <div class="opsi-box px-5 py-3 border border-gray-200 text-gray-700 rounded-xl transition min-w-[220px]">
                            <div class="flex flex-col">
                                ...
                                <span class="text-xs text-gray-500 mt-1.5 font-medium">Estimasi tiba <span class="text-gray-700 font-bold">{{ now()->addDays(5)->translatedFormat('d') }} - {{ now()->addDays(7)->translatedFormat('d M') }}</span></span>
            ...
            <div class="p-6 flex flex-col gap-6 bg-slate-50/50">
                ...
                <div>
                    ...
                    <div class="flex gap-3 flex-wrap">
                        <label class="relative select-none {{ $hasHandphone ? 'cursor-not-allowed opacity-50' : 'cursor-pointer' }}" 
                               @if($hasHandphone) title="COD tidak tersedia untuk pembelian Handphone" @endif>
                            <input type="radio" name="payment" value="cod" class="hidden" {{ !$hasHandphone ? 'checked' : 'disabled' }}>
                            <div class="opsi-box px-4 py-2.5 bg-white border {{ !$hasHandphone ? 'border-gray-300 text-gray-700 hover:border-orange-500 hover:text-orange-500' : 'border-gray-200 text-gray-400 bg-gray-50' }} rounded-lg transition">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mt-2">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <a href="{{ route('profile.bank') }}" class="text-[11px] font-bold text-blue-500 hover:underline">+ Tautkan Baru</a>
                        </div>
                        
                        @if(isset($savedEwallets) && $savedEwallets->count() > 0)
                            <div class="flex flex-col gap-3">
                                @foreach($savedEwallets as $ewallet)
                                <label class="relative cursor-pointer select-none w-full">
                                    <input type="radio" name="payment" value="ewallet_{{ $ewallet->id }}" class="hidden">
                                    <div class="opsi-box flex items-center bg-white px-4 py-3 border border-gray-200 rounded-xl text-gray-700 hover:border-orange-500 hover:text-orange-500 transition w-full shadow-sm">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="w-10 h-7 rounded flex items-center justify-center">
                                                ...
                                                @else
                                                    <span class="text-[10px] font-bold">{{ $ewallet->provider }}</span>
                                                @endif
                                            </div>
                                            <div>
                                                <p class="text-sm font-bold text-gray-800">
                                                    {{ $ewallet->provider }} •••• {{ substr($ewallet->account_number, -4) }}
                                                </p>
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <a href="{{ route('profile.bank') }}" class="text-[11px] font-bold text-blue-500 hover:underline">+ Tambah Kartu</a>
                        </div>

                        @if(isset($savedCards) && $savedCards->count() > 0)
                            <div class="flex flex-col gap-3">
                                @foreach($savedCards as $kartu)
                                <label class="relative cursor-pointer select-none w-full">
                                    <input type="radio" name="payment" value="kartu_{{ $kartu->id }}" class="hidden">
            ...
            <div class="p-6">
                ...
                <div class="space-y-3 text-sm">
                    <div class="flex justify-between items-center text-gray-600">
                        ...
                        <span class="font-medium text-gray-800">Rp {{ number_format($totalHarga ?? 0, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex justify-between items-center text-gray-600">
                    ...
                    <div class="flex justify-between items-center text-gray-600">
                        ...
                        <span class="font-medium text-gray-800">Rp {{ number_format($biayaLayanan ?? 0, 0, ',', '.') }}</span>
                    </div>
    ...
    <div class="fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 shadow-[0_-5px_15px_rgba(0,0,0,0.05)] z-50">
        <div class="max-w-5xl mx-auto flex justify-end items-stretch">
            ...
            <div class="flex items-stretch justify-end w-full md:w-auto">
                ...
                <form id="form-buat-pesanan" action="{{ route('checkout.process') }}" method="POST" class="flex">
                    @csrf
                    ...
                    <input type="hidden" name="alamat_id" value="{{ isset($alamatUtama) ? $alamatUtama->id : '' }}">
                    <input type="hidden" name="pengiriman" id="input-final-pengiriman" value="15000">
                    <input type="hidden" name="pembayaran" id="input-final-pembayaran" value="{{ !$hasHandphone ? 'cod' : '' }}">
   ...
   document.addEventListener('DOMContentLoaded', function() {
        const subtotalProduk = {{ $totalHarga ?? 0 }};
        const biayaLayanan = {{ $biayaLayanan ?? 0 }};
